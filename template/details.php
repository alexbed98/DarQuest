...
<div class="conteneur">
    <h2><?= $item !== false ? htmlspecialchars($item['nom']) : 'Item introuvable' ?></h2>
    ...
    <div id="quantiteStock">
        ...
        <p><?= $item !== false ? (int) $item['quantiteStock'] : '-' ?></p>
    </div>
    
    <div id="prix">
        <p>Prix: </p>
        <p id="prixItem" type="number"><?= $item !== false ? (float) $item['prix'] : '0' ?></p>
        <p>🪙</p>
    </div>
</div>
...
<div class="conteneur">
    ...
    <div class="conteneur" style="display: flex; justify-content: center; align-items: center;">
        <img src="<?= $item !== false ? htmlspecialchars($item['photo']) : 'image/photo.avif' ?>" alt="image item" style="height: 300px; width: 500px;">
    </div>

    <?php if ($item !== false): ?>
    <form method="post" action="" style="display: flex; flex-direction: row; justify-content: space-around; align-items: center; margin-top: 4em;">
        <input type="hidden" name="add_item_id" value="<?= (int) $item['idItem'] ?>">
        <p id="prixTotal"></p>
        <input type="number" name="qty" id="qtyInput" class="panier-qty-input" value="1" min="1">
        <button type="submit">Ajouter au panier</button>
    </form>
    <?php else: ?>
    <div style="margin-top: 4em; text-align: center;">
        <p>Item introuvable.</p>
    </div>
    <?php endif; ?>
...
<script>
    ...
    const inputQty = document.querySelector('#qtyInput');
